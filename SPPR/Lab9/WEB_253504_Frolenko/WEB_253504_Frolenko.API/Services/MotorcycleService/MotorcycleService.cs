     ...
     string? categoryNormalizedName, int pageNo = 1, int pageSize = 3)
        {
            try
            {
                ...
                {
                    var normalizedCategoryName = categoryNormalizedName.ToLower();
                    query = query.Where(d => d.Category != null &&
                        d.Category.NormalizedName.ToLower() == normalizedCategoryName);
                }
            ...
            {
                return ResponseData<ListModel<Motorcycle>>.Error("An error occurred while processing your request.");
            }
        }
