...
// Класс-наследник Resident
class Resident extends Person {
  constructor(lastName, city, street, houseNumber, apartmentNumber) {
    super(lastName, city); // Вызов конструктора базового класса
    this.street = street;
    this.houseNumber = houseNumber;
    this.apartmentNumber = apartmentNumber;
  }

  // Методы-геттеры и сеттеры для дополнительных свойств
  getStreet() {
    return this.street;
  }
...
function findMatchingResidents() {
  ...
  if (matches.length > 0) {
    // Отображаем все найденные совпадения
